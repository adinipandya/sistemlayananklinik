...
class PasienController extends Controller
{
    ...
    public function booking()
    {
        ...
        foreach ($dokters as $dokter) {
            $hariPraktek = strtolower($dokter->hari_praktek ?? '');

            $dokter->tersedia = $hariPraktek !== ''
                && str_contains($hariPraktek, $hariIni);
        }

        $dokterTersedia = $dokters->where('tersedia', true)->count();
    ...
    public function storeBooking(Request $request)
    {
        ...
        $dokterData = Dokter::findOrFail($request->dokter_id);

        $hariBooking = strtolower(
            \Carbon\Carbon::parse($request->tanggal)
                ->locale('id')
                ->translatedFormat('l')
        );

        $hariPraktek = strtolower($dokterData->hari_praktek ?? '');

        if ($hariPraktek === '' || !str_contains($hariPraktek, $hariBooking)) {
            return back()
                ->withInput()
                ->with('error', 'Dokter tidak praktik pada hari yang dipilih.');
        }
        ...
        $akunDokter = User::where('role', 'dokter')
            ->where('email', $dokterData->email)
            ->first();

        if ($akunDokter) {
            Notification::create([
                'user_id' => $akunDokter->id,
                'judul' => 'Booking Baru',
